...
class TransformerModel(tf.keras.Model):
    def __init__(self, num_heads, d_model, dff, input_shape, output_size, dropout_rate=0.1):
        super(TransformerModel, self).__init__()
        # 입력 데이터의 차원을 d_model로 변환
        ...
        self.final_layer = Dense(output_size)

    def call(self, inputs, training):
        # 입력 차원 맞추기
        inputs = self.dense_projection(inputs)
        ...
        return self.final_layer(out2[:, -1, :])  # 마지막 타임 스텝 예측


# 모델 인스턴스 생성 (예: 4개의 헤드, d_model = 64, FFN 차원 = 128)
d_model = 64
num_heads = 4
dff = 128
output_size = 1

model = TransformerModel(num_heads=num_heads, d_model=d_model, dff=dff, input_shape=(time_steps, x_train.shape[2]),
                         output_size=output_size)

# 모델 컴파일 (회귀 문제에 맞게 'mse' 사용)
